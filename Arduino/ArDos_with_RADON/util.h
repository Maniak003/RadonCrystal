...
uint32_t _get_aprox_back(float imp) //получить аппроксимированный фон
{
...
#else
#if GEIGER_DEAD_TIME
        return (DEFAULT_GEIGER_TIME / (pgm_read_float(&back_aprox_coef[i]) * ((1.0 / imp) - DEAD_TIME))); //рассчитываем фон в мкр/ч с поправкой на скорость счета и мертвое время
#else
        return (DEFAULT_GEIGER_TIME / (pgm_read_float(&back_aprox_coef[i]) * (1.0 / imp))); //рассчитываем фон в мкр/ч с поправкой на скорость счета
#endif
#endif
        break;
      }
  ...
  #else
  return (imp * DEFAULT_GEIGER_TIME); //рассчитываем фон в мкр/ч
  #endif
}
